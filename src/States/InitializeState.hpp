        ...
        struct InitializeState
                : StateInterface<SensorHiLinkZw0608>,
                  Will<
                      ...
                      On<InitOkReceivedEvent, TransitionTo<ReadyState> >,
                      On<ResetEvent, TransitionTo<ResetState> >
                  > {
            InitializeState(const char *name, SensorHiLinkZw0608 *machine, Stm32ItmLogger::LoggerInterface *logger)
                : StateInterface(name, machine, logger) { ; }

            using Will::handle;

            Status onEnter(const InitializeEvent &event);

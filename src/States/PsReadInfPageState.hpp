...
namespace Stm32Fingerprint {
    ...
    namespace States {
        struct PsReadInfPageState final
                : StateInterface<SensorHiLinkZw0608>,
                  ...
                  > {
            PsReadInfPageState(const char *name, SensorHiLinkZw0608 *machine, Stm32ItmLogger::LoggerInterface *logger)
                : StateInterface(name, machine, logger) { ; }

            using Will::handle;

            Status onEnter(const PsReadInfPageEvent &event);

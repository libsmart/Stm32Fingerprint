...
namespace Stm32Fingerprint {
    ...
    namespace States {
        struct PsReadSysParaState final
                : StateInterface<SensorHiLinkZw0608>,
                  ...
                  > {
            PsReadSysParaState(const char *name, SensorHiLinkZw0608 *machine, Stm32ItmLogger::LoggerInterface *logger)
                : StateInterface(name, machine, logger) { ; }

            using Will::handle;

            Status onEnter(const PsReadSysParaEvent &event);
